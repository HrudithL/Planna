...
if (command === 'test') {
  // Generate SQL for first 10 courses
  console.log('=== TEST MODE: First 10 courses ===\n');
  const sql = generateSqlForRange(0, 10);
  console.log(JSON.stringify(sql, null, 2));
  console.log(`\nGenerated ${sql.length} SQL statements for 10 courses`);
  
} else if (command === 'chunk') {
  // Generate SQL for a specific chunk
  const chunkIndex = parseInt(args[1] || '0');
  const chunkSize = parseInt(args[2] || '100');
  const start = chunkIndex * chunkSize;
  const end = Math.min(start + chunkSize, courses.length);
  
  console.log(`=== CHUNK ${chunkIndex}: Courses ${start + 1}-${end} ===\n`);
  const sql = generateSqlForRange(start, end);
  console.log(JSON.stringify(sql, null, 2));
  console.log(`\nGenerated ${sql.length} SQL statements`);
  
} else {
  // Show stats and instructions
  console.log(`Total courses: ${courses.length}`);
  console.log(`\nUsage:`);
  console.log(`  node scripts/import-to-neon.js test        - Generate SQL for first 10 courses`);
  console.log(`  node scripts/import-to-neon.js chunk [N] [SIZE] - Generate SQL for chunk N (size SIZE, default 100)`);
}
